...
@section('page_description', 'Kelola daftar perusahaan utama (Company) untuk tenant Anda.')
@section('page_actions')
    ...
    <x-button id="btn-add-company" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Perusahaan
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    ...
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="companies-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 45%;">Nama Perusahaan</th>
...
$(document).ready(function() {
    ...
    $('#btn-add-company').on('click', function() {
        ERPLoader.loadModal("{{ route('logistic.master.company.create') }}", '#addCompanyModal', {
            title: 'Tambah Perusahaan',
            errorMessage: 'Gagal memuat form tambah perusahaan.',
    ...
    var table = $('#companies-table').DataTable({
        ...
        orderCellsTop: true,
        ajax: {
            url: "{{ route('logistic.master.company.data') }}"
        },
    ...
    $('#companies-table thead tr.filter-row select.column-filter').select2({
        theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1
    });

    $('#companies-table thead tr.filter-row').on('click mousedown keydown', function(e) {
        e.stopPropagation();
    });

    $('#companies-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $(document).on('submit', '#add-company-form', function(e) {
        ...
        $.ajax({
            url: "{{ route('logistic.master.company.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function() {
        ...
        var url = "{{ url('logistic/master/company') }}/" + uuid;
        ERPLoader.loadModal(url, '#showCompanyModal', {
            title: 'Detail Perusahaan',
            errorMessage: 'Gagal mengambil data detail perusahaan.'
        });
    });

    $(document).on('click', '.edit-btn', function() {
        var uuid = $(this).data('uuid');
        var url = "{{ url('logistic/master/company') }}/" + uuid + "/edit";
        ERPLoader.loadModal(url, '#editCompanyModal', {
            title: 'Edit Perusahaan',
            errorMessage: 'Gagal mengambil data perusahaan.',
            onSuccess: function(modal) {
                $('#edit-status').select2({
                    theme: 'bootstrap-5',
                    dropdownParent: modal,
    ...
    $(document).on('submit', '#edit-company-form', function(e) {
        ...
        $.ajax({
            url: "{{ url('logistic/master/company') }}/" + uuid,
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Perusahaan?', 'Apakah Anda yakin ingin menghapus perusahaan "' + name + '"?').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() {
                        AppAlert.error('Gagal!', 'Gagal menghapus perusahaan.');
                    }
                });
            }
        });
