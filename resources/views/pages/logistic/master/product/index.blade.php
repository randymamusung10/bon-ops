...
@section('page_description', 'Kelola daftar produk dan layanan (Item Master).')
@section('page_actions')
    ...
    <x-button id="btn-add-product" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Produk
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="products-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 15%;">Kode Produk</th>
                        <th class="py-3" style="width: 20%;">Nama Produk</th>
                        <th class="py-3" style="width: 15%;">Kategori</th>
                        <th class="py-3" style="width: 10%;">Tipe</th>
                        <th class="py-3" style="width: 10%;">Harga Jual</th>
                    ...
                    <tr class="filter-row">
                        ...
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Kategori..."></th>
                        <th class="pb-3">
...
$(document).ready(function() {
    ...
    $('#btn-add-product').on('click', function() {
        ERPLoader.loadModal("{{ route('logistic.master.product.create') }}", '#addProductModal', {
            title: 'Tambah Produk',
            errorMessage: 'Gagal memuat form tambah produk.'
        });
    });
    ...
    var table = $('#products-table').DataTable({
        ...
        orderCellsTop: true,
        ajax: {
            url: "{{ route('logistic.master.product.data') }}"
        },
    ...
    $('#products-table thead tr.filter-row select.column-filter').select2({ theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1 });
    $('#products-table thead tr.filter-row').on('click mousedown keydown', function(e) { e.stopPropagation(); });

    $('#products-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $(document).on('submit', '#add-product-form', function(e) {
        ...
        $.ajax({
            url: "{{ route('logistic.master.product.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function() {
        ...
        var url = "{{ url('logistic/master/product') }}/" + uuid;
        ERPLoader.loadModal(url, '#showProductModal', { title: 'Detail Produk', errorMessage: 'Gagal mengambil data detail produk.' });
    });

    $(document).on('click', '.edit-btn', function() {
        var uuid = $(this).data('uuid');
        var url = "{{ url('logistic/master/product') }}/" + uuid + "/edit";
        ERPLoader.loadModal(url, '#editProductModal', { title: 'Edit Produk', errorMessage: 'Gagal mengambil data produk.' });
    });
    ...
    $(document).on('submit', '#edit-product-form', function(e) {
        ...
        $.ajax({
            url: "{{ url('logistic/master/product') }}/" + uuid,
            type: "POST", // Method SPOOFING included in form
            data: form.serialize(),
            success: function(response) {
                if (response.success) {
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Produk?', 'Apakah Anda yakin ingin menghapus produk "' + name + '"?').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() { AppAlert.error('Gagal!', 'Gagal menghapus produk.'); }
                });
            }
        });
    });
