...
@section('page_description', 'Kelola daftar hierarki kategori produk.')
@section('page_actions')
    ...
    <x-button id="btn-add-category" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Kategori
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="categories-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 25%;">Nama Kategori</th>
                        <th class="py-3" style="width: 25%;">Kategori Induk</th>
                    ...
                    <tr class="filter-row">
                        ...
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Induk..."></th>
                        <th class="pb-3">
...
$(document).ready(function() {
    ...
    $('#btn-add-category').on('click', function() {
        ERPLoader.loadModal("{{ route('logistic.master.category.create') }}", '#addCategoryModal', {
            title: 'Tambah Kategori',
            errorMessage: 'Gagal memuat form tambah kategori.'
        });
    });
    ...
    var table = $('#categories-table').DataTable({
        ...
        orderCellsTop: true,
        ajax: {
            url: "{{ route('logistic.master.category.data') }}"
        },
    ...
    $('#categories-table thead tr.filter-row select.column-filter').select2({ theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1 });
    $('#categories-table thead tr.filter-row').on('click mousedown keydown', function(e) { e.stopPropagation(); });

    $('#categories-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $(document).on('submit', '#add-category-form', function(e) {
        ...
        $.ajax({
            url: "{{ route('logistic.master.category.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function() {
        ...
        var url = "{{ url('logistic/master/category') }}/" + uuid;
        ERPLoader.loadModal(url, '#showCategoryModal', { title: 'Detail Kategori', errorMessage: 'Gagal mengambil data detail kategori.' });
    });

    $(document).on('click', '.edit-btn', function() {
        var uuid = $(this).data('uuid');
        var url = "{{ url('logistic/master/category') }}/" + uuid + "/edit";
        ERPLoader.loadModal(url, '#editCategoryModal', { title: 'Edit Kategori', errorMessage: 'Gagal mengambil data kategori.' });
    });
    ...
    $(document).on('submit', '#edit-category-form', function(e) {
        ...
        $.ajax({
            url: "{{ url('logistic/master/category') }}/" + uuid,
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Kategori?', 'Apakah Anda yakin ingin menghapus kategori "' + name + '"?').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() { AppAlert.error('Gagal!', 'Gagal menghapus kategori.'); }
                });
            }
        });
    });
