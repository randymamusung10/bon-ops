...
@section('page_description', 'Kelola daftar pelanggan untuk tenant Anda.')
@section('page_actions')
    ...
    <x-button id="btn-add-customer" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Pelanggan
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    ...
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="customers-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 25%;">Nama Pelanggan</th>
                        <th class="py-3" style="width: 15%;">Telepon</th>
                        <th class="py-3" style="width: 15%;">Email</th>
                    ...
                    <tr class="filter-row">
                        ...
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Pelanggan..."></th>
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Telepon..."></th>
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Email..."></th>
                        <th class="pb-3">
...
$(document).ready(function() {
    ...
    $('#btn-add-customer').on('click', function() {
        ERPLoader.loadModal("{{ route('logistic.master.customer.create') }}", '#addCustomerModal', {
            title: 'Tambah Pelanggan',
            errorMessage: 'Gagal memuat form tambah pelanggan.'
        });
    });
    ...
    var table = $('#customers-table').DataTable({
        ...
        orderCellsTop: true,
        ajax: {
            url: "{{ route('logistic.master.customer.data') }}"
        },
    ...
    $('#customers-table thead tr.filter-row select.column-filter').select2({
        theme: 'bootstrap-5',
        width: '100%',
        minimumResultsForSearch: -1
    });

    $('#customers-table thead tr.filter-row').on('click mousedown keydown', function(e) {
        e.stopPropagation();
    });

    $('#customers-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $(document).on('submit', '#add-customer-form', function(e) {
        ...
        $.ajax({
            url: "{{ route('logistic.master.customer.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function() {
        ...
        var url = "{{ url('logistic/master/customer') }}/" + uuid;
        
        ERPLoader.loadModal(url, '#showCustomerModal', {
            title: 'Detail Pelanggan',
            errorMessage: 'Gagal mengambil data detail pelanggan.'
        });
    });

    $(document).on('click', '.edit-btn', function() {
        var uuid = $(this).data('uuid');
        var url = "{{ url('logistic/master/customer') }}/" + uuid + "/edit";
        
        ERPLoader.loadModal(url, '#editCustomerModal', {
    ...
    $(document).on('submit', '#edit-customer-form', function(e) {
        ...
        $.ajax({
            url: "{{ url('logistic/master/customer') }}/" + uuid,
            type: "POST",
            data: form.serialize(),
            success: function(response) {
                if (response.success) {
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Pelanggan?', 'Apakah Anda yakin ingin menghapus pelanggan "' + name + '"?').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() {
                        AppAlert.error('Gagal!', 'Gagal menghapus pelanggan.');
                    }
                });
            }
        });
