...
@section('page_title', 'Manajemen Gudang (Warehouse)')
@section('page_description', 'Kelola daftar gudang untuk operasional Anda.')
@section('page_actions')
    ...
    <x-button id="btn-add-warehouse" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Gudang
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="warehouses-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 20%;">Nama Gudang</th>
                        <th class="py-3" style="width: 20%;">Cabang Terkait</th>
...
$(document).ready(function() {
    ...
    $('#btn-add-warehouse').on('click', function() {
        ERPLoader.loadModal("{{ route('logistic.master.warehouse.create') }}", '#addWarehouseModal', {
            title: 'Tambah Gudang',
            errorMessage: 'Gagal memuat form tambah gudang.'
        });
    });
    ...
    var table = $('#warehouses-table').DataTable({
        ...
        orderCellsTop: true,
        ajax: {
            url: "{{ route('logistic.master.warehouse.data') }}"
        },
    ...
    $('#warehouses-table thead tr.filter-row select.column-filter').select2({ theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1 });
    $('#warehouses-table thead tr.filter-row').on('click mousedown keydown', function(e) { e.stopPropagation(); });

    $('#warehouses-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $(document).on('submit', '#add-warehouse-form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        $.ajax({
            url: "{{ route('logistic.master.warehouse.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function() {
        ...
        var url = "{{ url('logistic/master/warehouse') }}/" + uuid;
        ERPLoader.loadModal(url, '#showWarehouseModal', { title: 'Detail Gudang', errorMessage: 'Gagal mengambil data detail gudang.' });
    });

    $(document).on('click', '.edit-btn', function() {
        var uuid = $(this).data('uuid');
        var url = "{{ url('logistic/master/warehouse') }}/" + uuid + "/edit";
        ERPLoader.loadModal(url, '#editWarehouseModal', { title: 'Edit Gudang', errorMessage: 'Gagal mengambil data gudang.' });
    });

    $(document).on('submit', '#edit-warehouse-form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        $.ajax({
            url: "{{ url('logistic/master/warehouse') }}/" + uuid,
            type: "POST",
            data: form.serialize(),
            success: function(response) {
                if (response.success) {
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Gudang?', 'Apakah Anda yakin ingin menghapus gudang "' + name + '"?').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() { AppAlert.error('Gagal!', 'Gagal menghapus gudang.'); }
                });
            }
        });
    });
