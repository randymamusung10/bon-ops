<x-modal id="editOpnameModal" title="Edit Stock Opname" description="Perbarui detail dokumen stock opname." size="xl">
    <form id="edit-opname-form" data-uuid="{{ $opname->uuid }}">
        @csrf
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <x-form.label required>Tanggal</x-form.label>
                <x-form.input type="date" name="date" id="edit-date" value="{{ $opname->date->format('Y-m-d') }}" required />
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" id="edit-branch_id" name="branch_id" required>
                    ...
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ $opname->branch_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" id="edit-warehouse_id" name="warehouse_id" required>
                    ...
                    @foreach($warehouses as $wh)
                        <option value="{{ $wh->id }}" {{ $opname->warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-12">
                ...
                <x-form.textarea name="notes" id="edit-notes" rows="2" placeholder="Catatan opsional mengenai stock opname ini">{{ $opname->notes }}</x-form.textarea>
            ...
            <table class="table table-hover align-middle mb-0" id="items-table" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent); border-bottom: 1px solid rgba(226, 232, 240, 0.2);">
                    <tr class="text-muted" style="letter-spacing: 0.2px;">
                        <th width="45%" class="py-3 ps-4 border-0">Produk</th>
                ...
                <tbody class="border-top-0 text-heading">
                    @foreach($opname->items as $index => $item)
                        <tr>
                            ...
                            <td class="py-3">
                                <input type="text" class="form-control form-control-sm format-number" name="items[{{ $index }}][actual_qty]" value="{{ (float)$item->actual_qty }}" required placeholder="0">
                            </td>
                            <td class="py-3">
                                <input type="text" class="form-control form-control-sm" name="items[{{ $index }}][notes]" value="{{ $item->notes }}" placeholder="Keterangan (Opsional)">
                            </td>
...
<template id="item-row-template-edit">
    <tr>
        ...
        <td class="py-3">
            <input type="text" class="form-control form-control-sm" name="items[__INDEX__][notes]" placeholder="Keterangan (Opsional)">
        </td>
        <td class="text-center py-3 pe-4">
            <button type="button" class="btn-icon-modern text-danger btn-remove-item mx-auto" title="Hapus" style="background: rgba(239, 68, 68, 0.12);">
                <i class="bi bi-trash"></i>
