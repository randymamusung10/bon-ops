<x-modal id="showOpnameModal" title="Detail Stock Opname" description="Dokumen {{ $opname->document_number }}" size="xl">
            ...
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ $opname->date->format('d/m/Y') }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span>
                            ...
                            @else
                                <span class="badge bg-dark px-2.5 py-1 rounded-pill" style="font-size: 11px;">{{ strtoupper($opname->status) }}</span>
                            @endif
                        </span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted fw-medium" style="font-size: 12px;">Dibuat Oleh</span>
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ $opname->creator->name ?? '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ $opname->branch->name }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted fw-medium" style="font-size: 12px;">Gudang</span>
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ $opname->warehouse->name }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-start">
                        ...
                        <span class="text-heading fw-semibold text-end" style="font-size: 13px;">
                            {{ $opname->poster->name ?? '-' }} 
                            @if($opname->posted_at)
                                <br><span class="text-muted fw-normal" style="font-size: 11px;">({{ $opname->posted_at->format('d/m/Y H:i') }})</span>
                            @endif
                        </span>
                    </div>
                </div>
        ...
        <div class="col-12">
            <div class="p-3 rounded-4 mt-1" style="background: rgba(14, 165, 233, 0.05); border: 1px dashed rgba(14, 165, 233, 0.3);">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div>
                        <p class="mb-1 text-muted fw-medium" style="font-size: 12px;">Catatan Opname:</p>
                        <p class="mb-0 text-heading fw-semibold" style="font-size: 13px; line-height: 1.5;">{{ $opname->notes }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="d-flex align-items-center mb-3">
        ...
        <h6 class="fw-bold mb-0 text-heading" style="letter-spacing: 0.3px;">Daftar Item Stock Opname</h6>
    </div>
        ...
        <table class="table table-hover align-middle mb-0" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
            <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent); border-bottom: 1px solid rgba(226, 232, 240, 0.2);">
                <tr class="text-muted" style="letter-spacing: 0.2px;">
                    ...
                    <th width="15%" class="text-end py-3 border-0">Stok Sistem</th>
                    <th width="15%" class="text-end py-3 border-0">Stok Fisik</th>
                    <th width="15%" class="text-end py-3 border-0">Selisih</th>
                    <th width="15%" class="py-3 pe-4 border-0">Keterangan</th>
                </tr>
            </thead>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top" style="border-color: rgba(226, 232, 240, 0.2) !important;">
        ...
        @if($opname->status === 'draft')
        <x-button type="button" variant="warning" size="sm" class="btn-action-opname" data-uuid="{{ $opname->uuid }}" data-action="submit" icon="bi-send">Ajukan (Submit)</x-button>
        @endif

        @if($opname->status === 'submitted')
        <x-button type="button" variant="primary" size="sm" class="btn-action-opname" data-uuid="{{ $opname->uuid }}" data-action="approve" icon="bi-check-lg">Setujui (Approve)</x-button>
        @endif

        @if($opname->status === 'approved')
        <x-button type="button" variant="success" size="sm" class="btn-action-opname" data-uuid="{{ $opname->uuid }}" data-action="post" icon="bi-check-circle-fill">Posting Opname</x-button>
        @endif
    </div>
</x-modal>
