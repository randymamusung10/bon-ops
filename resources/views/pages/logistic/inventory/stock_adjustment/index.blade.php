...
@section('page_description', 'Kelola dokumen penyesuaian stok, stok opname awal, atau koreksi barang.')
@section('page_actions')
    <x-button id="btn-export-excel" variant="ghost-success" size="sm" icon="bi-file-earmark-excel">
        Excel
    </x-button>
    <x-button id="btn-add-adjustment" variant="primary" size="sm" icon="bi-plus-lg">
        Buat Penyesuaian
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="adjustment-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
...
$(document).ready(function() {
    ...
    var table = $('#adjustment-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('logistic.inventory.adjustment.data') }}",
        orderCellsTop: true,
        order: [[2, 'desc']],
    ...
    $('#adjustment-table thead tr.filter-row select.column-filter').select2({
        theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1
    });

    $('#adjustment-table thead tr.filter-row').on('click mousedown keydown', function(e) {
        e.stopPropagation();
    });

    $('#adjustment-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $('#btn-add-adjustment').on('click', function(e) {
        e.preventDefault();
        ERPLoader.loadModal("{{ route('logistic.inventory.adjustment.create') }}", '#addAdjustmentModal', {
            title: 'Buat Penyesuaian Stok',
            errorMessage: 'Gagal memuat form penyesuaian stok.',
            onSuccess: function(modal) {
                ...
                function addItemRow() {
                    ...
                    $('#items-table tbody').find('select[name="items['+itemIndex+'][product_id]"]').select2({
                        theme: 'bootstrap-5',
                        dropdownParent: modal,
                        width: '100%'
                    });
    ...
    $(document).on('submit', '#add-adjustment-form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        $.ajax({
            url: "{{ route('logistic.inventory.adjustment.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Penyesuaian?', 'Dokumen yang masih berstatus draft akan dihapus permanen.').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('logistic/inventory/adjustment') }}/" + uuid,
                    type: "DELETE",
