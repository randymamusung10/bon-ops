...
@section('page_title', 'Waste (Pembuangan Stok)')
@section('page_description', 'Kelola pencatatan bahan baku atau produk yang terbuang, rusak, atau kedaluwarsa.')
@section('page_actions')
    <x-button id="btn-export-excel" variant="ghost-success" size="sm" icon="bi-file-earmark-excel">
        Excel
    </x-button>
    <x-button id="btn-add-waste" variant="primary" size="sm" icon="bi-plus-lg">
        Buat Waste
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="waste-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
...
$(document).ready(function() {
    ...
    var table = $('#waste-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('logistic.inventory.waste.data') }}",
        orderCellsTop: true,
        order: [[2, 'desc']],
        ...
        columns: [
            ...
            { data: 'uuid', name: 'uuid', class: 'pe-4 text-end text-nowrap', orderable: false, searchable: false, render: function(data, type, row) {
                let actions = '<div class="d-inline-flex gap-2">' +
                    '<button class="btn-icon-modern text-info show-btn" href="{{ url("logistic/inventory/waste") }}/'+data+'" title="Detail" style="background: rgba(14, 165, 233, 0.12);">' +
                    '<i class="bi bi-eye"></i>' +
                    '</button>';
                if (row.status === 'draft') {
                    actions += '<button class="btn-icon-modern text-primary edit-btn" href="{{ url("logistic/inventory/waste") }}/'+data+'/edit" title="Edit" style="background: rgba(59, 130, 246, 0.12);">' +
    ...
    $('#waste-table thead tr.filter-row select.column-filter').select2({
        theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1
    });

    $('#waste-table thead tr.filter-row').on('click mousedown keydown', function(e) {
        e.stopPropagation();
    });

    $('#waste-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $('#btn-add-waste').on('click', function(e) {
        e.preventDefault();
        ERPLoader.loadModal("{{ route('logistic.inventory.waste.create') }}", '#addWasteModal', {
            title: 'Buat Stock Waste',
            errorMessage: 'Gagal memuat form stock waste.',
            onSuccess: function(modal) {
                ...
                function addItemRow() {
                    ...
                    modal.find('select[name="items['+itemIndex+'][product_id]"]').select2({
                        theme: 'bootstrap-5',
                        dropdownParent: modal,
                        width: '100%'
                    });
    ...
    $(document).on('submit', '#add-waste-form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        $.ajax({
            url: "{{ route('logistic.inventory.waste.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function(e) {
        ...
        ERPLoader.loadModal(url, '#showWasteModal', {
            title: 'Detail Stock Waste',
            errorMessage: 'Gagal mengambil data stock waste.'
        });
    });
    ...
    $(document).on('click', '.edit-btn', function(e) {
        e.preventDefault();
        var url = $(this).attr('href');
        ERPLoader.loadModal(url, '#editWasteModal', {
            title: 'Edit Stock Waste',
            errorMessage: 'Gagal mengambil data stock waste.',
            onSuccess: function(modal) {
    ...
    $(document).on('submit', '#edit-waste-form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        $.ajax({
            url: "{{ url('logistic/inventory/waste') }}/" + uuid,
            type: "PUT",
            data: form.serialize(),
            success: function(response) {
                if (response.success) {
    ...
    $(document).on('click', '.btn-action-waste', function() {
        var uuid = $(this).data('uuid');
        var action = $(this).data('action');
        var textMap = {
            'submit': 'mengajukan dokumen stock waste ini',
            'approve': 'menyetujui dokumen stock waste ini',
            'post': 'mem-posting dokumen stock waste ini dan mengurangi stok fisik'
        };

        AppAlert.confirm('Konfirmasi Aksi', 'Apakah Anda yakin ingin ' + textMap[action] + '?', 'Ya, Lanjutkan!').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: `/logistic/inventory/waste/${uuid}/${action}`,
                    type: 'POST',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(res) {
                        $('#showWasteModal').modal('hide');
                        table.ajax.reload();
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Stock Waste?', 'Dokumen yang masih berstatus draft akan dihapus permanen.').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('logistic/inventory/waste') }}/" + uuid,
                    type: "DELETE",
