<x-modal id="editWasteModal" title="Edit Stock Waste" description="Perbarui detail dokumen stock waste." size="xl">
    <form id="edit-waste-form" data-uuid="{{ $waste->uuid }}">
        @csrf
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <x-form.label required>Tanggal</x-form.label>
                <x-form.input type="date" name="date" id="edit-date" value="{{ $waste->date->format('Y-m-d') }}" required />
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" id="edit-branch_id" name="branch_id" required>
                    ...
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ $waste->branch_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" id="edit-warehouse_id" name="warehouse_id" required>
                    ...
                    @foreach($warehouses as $wh)
                        <option value="{{ $wh->id }}" {{ $waste->warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-12">
                ...
                <x-form.textarea name="notes" id="edit-notes" rows="2" placeholder="Catatan umum mengenai pembuangan stok ini">{{ $waste->notes }}</x-form.textarea>
        ...
        <div class="table-responsive rounded-4 overflow-hidden mb-3" style="border: 1px solid rgba(226, 232, 240, 0.2);">
            <table class="table table-hover align-middle mb-0" id="items-table" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
                ...
                <tbody class="border-top-0 text-heading">
                    @foreach($waste->items as $index => $item)
                        <tr>
                            <td class="py-3 ps-4">
                            ...
                            <td class="py-3">
                                <input type="text" class="form-control form-control-sm format-number" name="items[{{ $index }}][qty]" value="{{ (float)$item->qty }}" required placeholder="0">
                            </td>
                            <td class="py-3">
                                <select class="form-select form-select-sm" name="items[{{ $index }}][reason]" required>
                                    <option value="Expired" {{ $item->reason == 'Expired' ? 'selected' : '' }}>Expired (Kedaluwarsa)</option>
                                    <option value="Spoiled" {{ $item->reason == 'Spoiled' ? 'selected' : '' }}>Spoiled (Busuk/Basi)</option>
                                    <option value="Damaged" {{ $item->reason == 'Damaged' ? 'selected' : '' }}>Damaged (Rusak/Pecah)</option>
                                    <option value="Sample" {{ $item->reason == 'Sample' ? 'selected' : '' }}>Sample (Uji Coba/Trial)</option>
                                    <option value="Other" {{ $item->reason == 'Other' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                            </td>
...
<!-- Hidden Template for new row edit -->
