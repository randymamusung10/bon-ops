<x-modal id="showModal" title="Detail Penerimaan Barang" description="Dokumen {{ $receipt->document_number }}" size="xl">
    <div class="row g-3 mb-4">
            ...
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted fw-medium" style="font-size: 12px;">Tanggal Penerimaan</span>
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ \Carbon\Carbon::parse($receipt->date)->format('d/m/Y') }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted fw-medium" style="font-size: 12px;">Nomor PO Reference</span>
                        <span class="text-heading fw-semibold text-primary" style="font-size: 13px;">{{ $receipt->purchaseOrder->po_number ?? '-' }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span>
                            ...
                            @elseif($receipt->status === 'submitted') 
                            ...
                            @else 
                                <span class="badge bg-dark px-2.5 py-1 rounded-pill" style="font-size: 11px;">{{ $receipt->status }}</span> 
                            @endif
                        </span>
                    </div>
                </div>
        ...
        <div class="col-md-6">
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted fw-medium" style="font-size: 12px;">Gudang Penerima</span>
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ $receipt->warehouse->name }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted fw-medium" style="font-size: 12px;">Supplier Pengirim</span>
                        <span class="text-heading fw-semibold" style="font-size: 13px;">{{ $receipt->supplier->name ?? '-' }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-start mt-2">
                        ...
                        <span class="text-heading fw-semibold text-end" style="font-size: 13px;">
                            Penerima: {{ $receipt->creator->name ?? '-' }}<br>
                            @if($receipt->approver)
                                Disetujui: {{ $receipt->approver->name }}
                            @endif
                            @if($receipt->poster)
                                <br>Diposting: {{ $receipt->poster->name }}
                            @endif
                        </span>
                    </div>
                </div>
        ...
        <div class="col-12">
            <div class="p-3 rounded-4 mt-1" style="background: rgba(14, 165, 233, 0.05); border: 1px dashed rgba(14, 165, 233, 0.3);">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div>
                        <p class="mb-1 text-muted fw-medium" style="font-size: 12px;">Catatan Penerimaan:</p>
                        <p class="mb-0 text-heading fw-semibold" style="font-size: 13px; line-height: 1.5;">{{ $receipt->notes }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="d-flex align-items-center mb-3">
        ...
        <h6 class="fw-bold mb-0 text-heading" style="letter-spacing: 0.3px;">Rincian Barang Diterima</h6>
    </div>
        ...
        <table class="table table-hover align-middle mb-0" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
            ...
            <tbody class="border-top-0 text-heading">
                ...
                <tr>
                    ...
                    <td class="text-end py-3 text-muted">{{ number_format($item->ordered_qty, 2, ',', '.') }}</td>
                    <td class="text-end py-3 fw-bold {{ $item->received_qty < $item->ordered_qty ? 'text-warning' : 'text-primary' }}">{{ number_format($item->received_qty, 2, ',', '.') }}</td>
                    <td class="py-3 pe-4 text-end text-muted">{{ $item->notes ?? '-' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top" style="border-color: rgba(226, 232, 240, 0.2) !important;">
        ...
        @if($receipt->status === 'draft')
        <x-button type="button" variant="warning" size="sm" class="btn-action-receipt" data-uuid="{{ $receipt->uuid }}" data-action="submit" icon="bi-send">Ajukan (Submit)</x-button>
        @endif

        @if($receipt->status === 'submitted')
        <x-button type="button" variant="primary" size="sm" class="btn-action-receipt" data-uuid="{{ $receipt->uuid }}" data-action="approve" icon="bi-check-lg">Setujui (Approve)</x-button>
        @endif

        @if($receipt->status === 'approved')
        <x-button type="button" variant="success" size="sm" class="btn-action-receipt" data-uuid="{{ $receipt->uuid }}" data-action="post" icon="bi-check-circle-fill">Posting GR (Tambah Stok)</x-button>
        @endif
    </div>
</x-modal>
