...
@section('page_description', 'Pemantauan antrean pesanan makanan secara real-time untuk Dapur Utama.')

@section('content')
<div class="container-fluid px-0">
    <!-- Grid of orders -->
    <div class="row g-4" id="kds-wrapper">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start">
                                        ...
                                        <div>
                                            ...
                                            @elseif($item->status === 'cooking')
                                                <span class="badge bg-info-subtle text-info px-2 py-0.5 rounded-pill" style="font-size: 10px;">Memasak</span>
                                            @endif
                                        </div>
                                    </div>
                                    @if($item->notes)
        ...
        @empty
            <div class="col-12 text-center py-5">
                <div class="text-muted mb-3"><i class="bi bi-fire" style="font-size: 48px; color: var(--primary-accent);"></i></div>
                <h5 class="fw-bold text-heading" style="font-family: 'Outfit', sans-serif;">Tidak Ada Pesanan Dapur</h5>
                <p class="text-muted" style="font-size: 13px;">Semua pesanan makanan telah selesai disajikan.</p>
            </div>
        @endforelse
    </div>
</div>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-status-update', function() {
        ...
        $.ajax({
            url: `/operational/restaurant/kitchen/${id}/status`,
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                status: status
