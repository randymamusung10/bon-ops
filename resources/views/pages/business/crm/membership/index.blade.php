...
@section('page_title', 'Membership Pelanggan')
@section('page_description', 'Kelola tingkatan keanggotaan dan diskon pelanggan.')
@section('page_actions')
    <x-button id="btn-add-membership" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Membership
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    ...
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="membership-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 25%;">Nama Membership</th>
                        <th class="py-3" style="width: 20%;">Minimum Spend</th>
                        <th class="py-3" style="width: 15%;">Diskon (%)</th>
                    ...
                    <tr class="filter-row">
                        ...
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Min Spend..."></th>
                        <th class="pb-3"><input type="text" class="form-control form-control-sm column-filter" placeholder="Cari Diskon..."></th>
                        <th class="pb-3">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn-add-membership').on('click', function() {
            ERPLoader.loadModal("{{ route('business.crm.membership.create') }}", '#addMembershipModal', {
                title: 'Tambah Membership',
                errorMessage: 'Gagal memuat form tambah membership.',
        ...
        table = $('#membership-table').DataTable({
            processing: true,
            serverSide: true,
            orderCellsTop: true,
            ajax: "{{ route('business.crm.membership.data') }}",
        ...
        $('#membership-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
        ...
        $('#membership-table thead tr.filter-row').on('click mousedown keydown', function(e) { e.stopPropagation(); });

        $(document).on('submit', '#add-membership-form', function(e) {
            e.preventDefault();
            let form = $(this);
            ...
            $.ajax({
                url: "{{ route('business.crm.membership.store') }}",
                method: "POST",
                data: form.serialize(),
        ...
        $(document).on('click', '.edit-btn', function() {
            var id = $(this).data('id');
            var url = "{{ url('business/crm/membership') }}/" + id + "/edit";
            
            ERPLoader.loadModal(url, '#editMembershipModal', {
        ...
        $(document).on('submit', '#edit-membership-form', function(e) {
            e.preventDefault();
            let form = $(this);
            let btn = form.find('button[type="submit"]');
            btn.prop('disabled', true);
            ...
            $.ajax({
                url: "{{ url('business/crm/membership') }}/" + id,
                type: "PUT",
                data: form.serialize(),
                success: function(response) {
                    if (response.success) {
        ...
        $(document).on('click', '.delete-btn', function() {
            var id = $(this).data('id');
            AppAlert.confirmDelete('Hapus Membership?', 'Data yang dihapus tidak dapat dikembalikan!').then((result) => {
