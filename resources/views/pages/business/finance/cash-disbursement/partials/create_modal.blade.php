<x-modal id="createModal" title="Buat Pengeluaran Kas Baru" size="xl">
    <form id="form-create-disbursement" enctype="multipart/form-data">
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-4">
                <x-form.label required>Dikeluarkan Dari (Akun Kas/Bank)</x-form.label>
                <x-form.select name="account_id" id="create-account-id" class="select2-modal" style="width: 100%;" required>
            ...
            <div class="col-md-4">
                ...
                <x-form.input type="text" name="reference_number" placeholder="Contoh: INV-SUP-001 / Struk Beli" />
            ...
            <div class="col-md-8 d-flex flex-column">
                ...
                <x-form.textarea name="description" class="flex-grow-1" style="resize: none;" placeholder="Catatan transaksi..."></x-form.textarea>
            </div>
            <div class="col-md-4">
                ...
                <div class="position-relative p-3 text-center rounded-3 bg-light hover-shadow-sm" style="border: 1px dashed #cbd5e1; transition: all 0.2s;" id="dropzone-create">
                    ...
                    <div class="d-flex flex-column align-items-center justify-content-center">
                        ...
                        <span class="text-muted fw-medium" style="font-size: 11px;" id="file-name-create">Pilih atau Letakkan File</span>
            ...
            <table class="table align-middle mb-0" id="items-table" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent); border-bottom: 1px solid rgba(226, 232, 240, 0.2);">
                    <tr class="text-muted" style="letter-spacing: 0.2px;">
                        <th width="35%" class="py-2 ps-4 border-0">Tujuan Dana (Akun) <span class="text-danger">*</span></th>
                ...
                <tbody class="border-top-0">
                    <tr>
                        <td class="py-2 ps-4">
                            <x-form.select name="items[0][account_id]" class="account-select" style="width: 100%;" required>
                        ...
                        <td class="py-2"><x-form.input size="sm" type="text" name="items[0][description]" placeholder="Contoh: Biaya Listrik, Beli Galon..." required /></td>
                        <td class="py-2"><x-form.input size="sm" type="text" name="items[0][amount]" class="text-end input-amount format-number" value="0" required /></td>
                ...
                <tfoot>
                    <tr style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                        <td colspan="2" class="py-2 text-end fw-bold">Total Pengeluaran (Rp)</td>
                        <td class="text-end py-3 pe-4 fw-bold text-primary"><span id="total-amount">0</span></td>
        ...
        <div class="d-flex justify-content-end gap-2 pt-3 border-top" style="border-color: rgba(226, 232, 240, 0.2) !important;">
            ...
            <x-button type="button" variant="primary" size="sm" id="btn-save-disbursement" icon="bi-check2">Simpan Draft</x-button>
        </div>
    </form>
</x-modal>
...
$(document).ready(function() {
    ...
    $('#create-account-id').select2({ theme: 'bootstrap-5', dropdownParent: $('#createModal') });
    $('.account-select').select2({ theme: 'bootstrap-5', dropdownParent: $('#createModal') });
    window.AppFormat.init();

    function calculateTotal() {
        let total = 0;
        $('#items-table tbody tr').each(function() {
            let amountStr = window.AppFormat.unmaskNumber($(this).find('.input-amount').val());
            let val = parseFloat(amountStr) || 0;
            total += val;
        });
        $('#total-amount').text(window.AppFormat.formatRupiah(total));
        
        if (total <= 0) {
            $('#btn-save-disbursement').prop('disabled', true);
        } else {
            $('#btn-save-disbursement').prop('disabled', false);
        }
    }
    ...
    $('#btn-add-item').on('click', function() {
        ...
        <tr>
            <td class="py-2 ps-4">
                <x-form.select name="items[${itemIndex}][account_id]" class="account-select" style="width: 100%;" required>
                    <option value="">Pilih Akun Tujuan</option>
                    @foreach($otherAccounts as $acc)
                        <option value="{{ $acc->id }}">{{ $acc->code }} - {{ $acc->name }}</option>
                    @endforeach
            ...
            <td class="py-2"><x-form.input size="sm" type="text" name="items[${itemIndex}][amount]" class="text-end input-amount format-number" value="0" required /></td>
            <td class="text-center py-2 pe-4">
                <button type="button" class="btn-icon-modern text-danger btn-remove-item mx-auto" title="Hapus" style="background: rgba(239, 68, 68, 0.12);"><i class="bi bi-trash"></i></button>
            </td>
        </tr>
        ...
        $('#items-table tbody tr:last .account-select').select2({ theme: 'bootstrap-5', dropdownParent: $('#createModal') });
        window.AppFormat.init();
        itemIndex++;
        calculateTotal();
    });
    ...
    $('#btn-save-disbursement').on('click', function() {
        let form = $('#form-create-disbursement');
        ...
        let total = window.AppFormat.unmaskNumber($('#total-amount').text());
        if (total <= 0) {
            AppAlert.warning('Total Tidak Valid', 'Total Pengeluaran harus lebih besar dari 0.');
            return;
        }
        ...
        $.ajax({
            url: "{{ route('business.finance.cash_disbursement.store') }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
