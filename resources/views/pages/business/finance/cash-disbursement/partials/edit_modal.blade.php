<x-modal id="editModal" title="Edit Pengeluaran Kas: {{ $transaction->transaction_number }}" size="xl">
    <form id="form-edit-disbursement" enctype="multipart/form-data">
        @method('PUT')
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-4">
                ...
                <x-form.input type="text" name="reference_number" value="{{ $transaction->reference_number }}" placeholder="Contoh: INV-SUP-001 / Struk Beli" />
            ...
            <div class="col-md-4">
                ...
                <div class="position-relative p-3 text-center rounded-3 bg-light hover-shadow-sm" style="border: 1px dashed {{ $transaction->attachment_path ? 'var(--bs-danger)' : '#cbd5e1' }}; background-color: {{ $transaction->attachment_path ? 'rgba(var(--bs-danger-rgb), 0.03)' : 'var(--bs-light)' }}; transition: all 0.2s;" id="dropzone-edit">
                    ...
                    <div class="d-flex flex-column align-items-center justify-content-center">
                        <i class="bi bi-cloud-arrow-up text-danger mb-1" style="font-size: 1.2rem;"></i>
                        <span class="{{ $transaction->attachment_path ? 'text-danger' : 'text-muted' }} fw-medium" style="font-size: 11px;" id="file-name-edit">
                ...
                @if($transaction->attachment_path)
                    <div class="mt-2 text-end">
                        <a href="{{ Storage::url($transaction->attachment_path) }}" target="_blank" class="btn btn-sm btn-outline-danger py-1 px-2" style="font-size: 11px;">
            ...
            <table class="table align-middle mb-0" id="items-table-edit" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
                ...
                <tbody class="border-top-0">
                    ...
                    <tr>
                        ...
                        <td class="py-2"><x-form.input size="sm" type="text" name="items[{{$index}}][description]" value="{{ $item->description }}" placeholder="Contoh: Biaya Listrik, Beli Galon..." required /></td>
        ...
        <div class="d-flex justify-content-end gap-2 pt-3 border-top" style="border-color: rgba(226, 232, 240, 0.2) !important;">
            ...
            <x-button type="button" variant="primary" size="sm" id="btn-update-disbursement" icon="bi-check2">Simpan Perubahan</x-button>
        </div>
    </form>
</x-modal>
...
$(document).ready(function() {
    ...
    function calculateTotalEdit() {
        ...
        if (total <= 0) {
            $('#btn-update-disbursement').prop('disabled', true);
        } else {
            $('#btn-update-disbursement').prop('disabled', false);
        }
    }
    ...
    $('#btn-update-disbursement').on('click', function() {
        let form = $('#form-edit-disbursement');
        ...
        $.ajax({
            url: "{{ route('business.finance.cash_disbursement.update', $transaction->uuid) }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
