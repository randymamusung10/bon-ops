<x-modal id="showModal" title="Detail Jurnal Umum" description="Dokumen {{ $journal->journal_number }}" size="xl">
        ...
        <div class="col-md-6">
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <table class="table table-borderless table-sm mb-0" style="font-size: 13px;">
                    <tbody>
                        <tr>
                            <td class="text-muted fw-medium pb-2" width="40%">Tanggal Jurnal</td>
                            <td class="fw-semibold text-heading pb-2">{{ \Carbon\Carbon::parse($journal->date)->format('d/m/Y') }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted fw-medium pb-2">Tipe Referensi</td>
                            <td class="fw-semibold text-heading pb-2">{{ $journal->reference_type ?: '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted fw-medium pb-0">ID Referensi</td>
                            <td class="fw-semibold text-heading pb-0">{{ $journal->reference_id ?: '-' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="col-md-6">
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <table class="table table-borderless table-sm mb-0" style="font-size: 13px;">
                    <tbody>
                        <tr>
                            <td class="text-muted fw-medium pb-2" width="40%">Cabang</td>
                            <td class="fw-semibold text-heading pb-2">{{ $journal->branch->name ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted fw-medium pb-2">Dibuat Oleh</td>
                            <td class="fw-semibold text-heading pb-2">{{ $journal->creator->name ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted fw-medium pb-0">Mata Uang</td>
                            <td class="fw-semibold text-heading pb-0">IDR - Rupiah (Base)</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <div class="row g-4 mb-2">
        ...
        <div class="col-md-4">
            @if($journal->attachment_path)
                ...
                <div class="p-3 rounded-4 h-100 d-flex flex-column justify-content-center" style="background: rgba(var(--bs-primary-rgb), 0.03); border: 1px dashed rgba(var(--bs-primary-rgb), 0.2);">
                    ...
                    <div class="d-flex gap-2">
                        <button type="button" onclick="previewAttachment('{{ Storage::url($journal->attachment_path) }}')" class="btn btn-primary btn-sm flex-grow-1 rounded-3 d-flex justify-content-center align-items-center gap-2">
                            <i class="bi bi-eye"></i> Lihat
                        </button>
                        <a href="{{ Storage::url($journal->attachment_path) }}" target="_blank" class="btn btn-outline-primary btn-sm rounded-3 px-3" title="Unduh File" download>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top" style="border-color: rgba(226, 232, 240, 0.2) !important;">
        @if(in_array($journal->status, ['approved', 'posted']))
            <a href="{{ route('business.finance.journal.print', $journal->uuid) }}" target="_blank" class="btn btn-outline-primary btn-sm custom-btn rounded-3 px-3 me-auto">
        ...
        @if($journal->status === 'draft')
            <x-button type="button" variant="warning" size="sm" class="btn-action-journal" data-uuid="{{ $journal->uuid }}" data-action="submit" icon="bi-send">
                Ajukan (Submit)
            </x-button>
        @elseif($journal->status === 'submitted')
        ...
        @elseif($journal->status === 'approved')
            <x-button type="button" variant="success" size="sm" class="btn-action-journal" data-uuid="{{ $journal->uuid }}" data-action="post" icon="bi-check-circle-fill">
                Posting Jurnal
            </x-button>
        @endif
    </div>
