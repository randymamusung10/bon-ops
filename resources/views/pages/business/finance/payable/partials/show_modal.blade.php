<x-modal id="show-modal-payable" title="Detail Hutang" description="Informasi lengkap mengenai tagihan hutang dan riwayat pembayaran." size="xl">
    ...
    <div class="d-flex align-items-center mb-4 p-3 rounded-4" style="background: rgba(14, 165, 233, 0.05); border: 1px solid rgba(14, 165, 233, 0.1);">
        ...
        <div class="ms-3">
            <h5 class="mb-1 fw-bold" style="color: var(--text-heading);">Invoice: {{ $invoice->supplier_invoice_number }}</h5>
            <div class="d-flex align-items-center gap-2">
                <span class="badge bg-secondary-subtle text-secondary px-2 py-1"><i class="bi bi-calendar3 me-1"></i>{{ \Carbon\Carbon::parse($invoice->date)->format('d/m/Y') }}</span>
                @php
                ...
                <span class="badge bg-info-subtle text-info px-2 py-1">
                    <i class="bi bi-shop me-1"></i>{{ $invoice->supplier->name ?? 'Supplier' }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-md-7">
            <div class="row g-3">
                <div class="col-sm-6">
                    <div class="p-3 rounded-3" style="background: var(--bg-dark-secondary); border: 1px solid var(--border-color);">
                        <div class="text-muted mb-1" style="font-size: 13px; font-weight: 500; letter-spacing: 0.2px;">
                            <i class="bi bi-calendar-x me-1"></i> Jatuh Tempo
                        </div>
                ...
                <div class="col-sm-6">
                    <div class="p-3 rounded-3" style="background: var(--bg-dark-secondary); border: 1px solid var(--border-color);">
                        <div class="text-muted mb-1" style="font-size: 13px; font-weight: 500; letter-spacing: 0.2px;">
                            <i class="bi bi-file-text me-1"></i> Referensi PO
                        </div>
                        <div class="fw-semibold text-heading" style="font-size: 14px;">
                ...
                <div class="col-12">
                    <div class="p-3 rounded-3 h-100" style="background: var(--bg-dark-secondary); border: 1px solid var(--border-color);">
                        <div class="text-muted mb-1" style="font-size: 13px; font-weight: 500; letter-spacing: 0.2px;">
                            <i class="bi bi-card-text me-1"></i> Keterangan Invoice
                        </div>
                        <div class="fw-medium text-heading" style="font-size: 14px; line-height: 1.5;">
                            {{ $invoice->notes ?: 'Tidak ada keterangan tambahan' }}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-5">
            @php
                ...
                $sisa = max(0, $invoice->grand_total - $totalPaid);
                $gradientBox = $isPaid 
    ...
    <div class="mt-4 pt-4" style="border-top: 1px dashed var(--border-color);">
        <h6 class="mb-3 fw-bold" style="color: var(--text-heading);"><i class="bi bi-box me-2 text-primary"></i>Detail Item Invoice</h6>
                ...
                <table class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                    ...
                    <tfoot style="font-size: 13px; color: var(--text-heading); border-top: 2px solid rgba(226, 232, 240, 0.8);">
                        <tr>
                            ...
                            <td class="text-end pe-4 fw-medium py-1">Rp {{ number_format($invoice->subtotal, 0, ',', '.') }}</td>
                        </tr>
                        @if($invoice->tax_amount > 0)
                        <tr>
                            <td colspan="3" class="text-end text-muted fw-medium py-1">Pajak (Tax):</td>
                            <td class="text-end pe-4 fw-medium py-1 text-danger">+ Rp {{ number_format($invoice->tax_amount, 0, ',', '.') }}</td>
                        </tr>
                        @endif
                        @if($invoice->discount_amount > 0)
                        <tr>
                            <td colspan="3" class="text-end text-muted fw-medium py-1">Diskon:</td>
                            <td class="text-end pe-4 fw-medium py-1 text-success">- Rp {{ number_format($invoice->discount_amount, 0, ',', '.') }}</td>
                        </tr>
                        @endif
                        <tr style="background-color: rgba(14, 165, 233, 0.05);">
                            ...
                            <td class="text-end pe-4 text-primary fw-bold py-2" style="font-size: 14px;">Rp {{ number_format($invoice->grand_total, 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                ...
                <table class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                    <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                        <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                            ...
                            <th class="py-2" style="width: 20%; font-weight: 600;">No. Bukti / Status</th>
                    ...
                    <tbody style="font-size: 13px; color: var(--text-heading);">
                        @foreach($invoice->payments as $payment)
                            <tr>
                                <td class="ps-4 fw-medium">{{ \Carbon\Carbon::parse($payment->payment_date)->format('d M Y') }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="fw-medium text-heading mb-1">{{ $payment->payment_number }}</div>
