...
@section('page_title', 'Mata Uang')
@section('page_description', 'Kelola daftar mata uang (Currency) untuk operasional multi-currency.')
@section('page_actions')
    ...
    <x-button id="btn-add-currency" variant="primary" size="sm" icon="bi-plus-lg">
        Tambah Mata Uang
    </x-button>
@endsection
...
<div class="container-fluid px-0">
    <div class="card rounded-4 border-0 shadow-sm p-4" style="background: var(--bg-dark-secondary);">
        <div class="table-responsive">
            <table id="currencies-table" class="table align-middle mb-0 w-100" style="--bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.6);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                    <tr style="font-size: 13px; color: var(--text-muted); letter-spacing: 0.2px;">
                        ...
                        <th class="py-3" style="width: 15%;">Kode Mata Uang</th>
                        <th class="py-3" style="width: 20%;">Nama Mata Uang</th>
                        <th class="py-3" style="width: 10%;">Simbol</th>
                        <th class="py-3" style="width: 20%;">Nilai Tukar (Base)</th>
...
$(document).ready(function() {
    ...
    $('#btn-add-currency').on('click', function() {
        ERPLoader.loadModal("{{ route('business.finance.currency.create') }}", '#addCurrencyModal', {
            title: 'Tambah Mata Uang',
            errorMessage: 'Gagal memuat form tambah mata uang.'
        });
    });
    ...
    var table = $('#currencies-table').DataTable({
        ...
        orderCellsTop: true,
        ajax: {
            url: "{{ route('business.finance.currency.data') }}"
        },
        ...
        columns: [
            ...
            { 
                data: 'exchange_rate', 
                name: 'exchange_rate',
                class: 'text-heading text-end pe-4',
    ...
    $('#currencies-table thead tr.filter-row select.column-filter').select2({ theme: 'bootstrap-5', width: '100%', minimumResultsForSearch: -1 });
    $('#currencies-table thead tr.filter-row').on('click mousedown keydown', function(e) { e.stopPropagation(); });

    $('#currencies-table thead tr.filter-row .column-filter').on('keyup change clear', function() {
    ...
    $(document).on('submit', '#add-currency-form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        $.ajax({
            url: "{{ route('business.finance.currency.store') }}",
            type: "POST",
            data: form.serialize(),
    ...
    $(document).on('click', '.show-btn', function() {
        ...
        var url = "{{ url('business/finance/currency') }}/" + uuid;
        ERPLoader.loadModal(url, '#showCurrencyModal', { title: 'Detail Mata Uang', errorMessage: 'Gagal mengambil data detail mata uang.' });
    });

    $(document).on('click', '.edit-btn', function() {
        var uuid = $(this).data('uuid');
        var url = "{{ url('business/finance/currency') }}/" + uuid + "/edit";
        ERPLoader.loadModal(url, '#editCurrencyModal', { title: 'Edit Mata Uang', errorMessage: 'Gagal mengambil data mata uang.' });
    });
    ...
    $(document).on('submit', '#edit-currency-form', function(e) {
        ...
        $.ajax({
            url: "{{ url('business/finance/currency') }}/" + uuid,
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Mata Uang?', 'Apakah Anda yakin ingin menghapus mata uang "' + name + '"?').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() { AppAlert.error('Gagal!', 'Gagal menghapus mata uang.'); }
                });
            }
        });
    });
