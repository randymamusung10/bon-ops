...
@section('page_title', 'Penerimaan Kas (Cash Receipt)')
@section('page_description', 'Kelola data penerimaan kas dan bank.')
...
$(document).ready(function() {
    ...
    var table = $('#receipt-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('business.finance.cash_receipt.data') }}",
        order: [[1, 'desc']],
        ...
        columns: [
            ...
            { data: 'action', name: 'action', orderable: false, searchable: false, class: 'pe-4 text-end text-nowrap', render: function(data, type, row) {
                ...
                let actions = '<div class="d-inline-flex gap-2">' +
                    '<button class="btn-icon-modern text-info show-btn" href="{{ url("business/finance/cash-receipt") }}/'+uuid+'" title="Detail" style="background: rgba(14, 165, 233, 0.12);">' +
                    '<i class="bi bi-eye"></i>' +
                    '</button>';
                if (row.status === 'draft') {
                    actions += '<button class="btn-icon-modern text-warning edit-btn" href="{{ url("business/finance/cash-receipt") }}/'+uuid+'/edit" title="Edit" style="background: rgba(245, 158, 11, 0.12);">' +
    ...
    $('#btn-add-receipt').on('click', function(e) {
        e.preventDefault();
        ERPLoader.loadModal("{{ route('business.finance.cash_receipt.create') }}", '#createModal', {
    ...
    $(document).on('click', '.btn-action-receipt', function() {
        ...
        AppAlert.confirm('Konfirmasi Aksi', 'Apakah Anda yakin ingin ' + textMap[action] + '?', 'Ya, Lanjutkan!').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: `/business/finance/cash-receipt/${uuid}/${action}`,
                    type: 'POST',
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        AppAlert.confirmDelete('Hapus Transaksi?', 'Draft transaksi akan dihapus secara permanen.').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('business/finance/cash-receipt') }}/" + uuid,
                    type: "DELETE",
                    data: { _token: "{{ csrf_token() }}" },
