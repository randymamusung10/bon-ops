<x-modal id="showModal" title="Detail Penerimaan Kas" description="Dokumen {{ $transaction->transaction_number }}" size="xl">
    <!-- Header Summary -->
        ...
        <div class="col-md-6">
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-sm-12">
                        <div class="text-muted fw-medium mb-1" style="font-size: 11px;">Disimpan Ke (Akun)</div>
        ...
        <div class="col-md-6">
            <div class="p-3 rounded-4 h-100" style="background: color-mix(in srgb, var(--primary-accent) 4%, transparent); border: 1px solid rgba(226, 232, 240, 0.1);">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-sm-12">
                        <div class="text-muted fw-medium mb-1" style="font-size: 11px;">Total Penerimaan</div>
                        <div class="fw-bold text-primary" style="font-size: 15px;">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="table table-bordered table-hover align-middle mb-0" style="font-size: 13px; --bs-table-bg: transparent; --bs-table-border-color: rgba(226, 232, 240, 0.2);">
                <thead style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent); border-bottom: 1px solid rgba(226, 232, 240, 0.2);">
                    <tr class="text-muted" style="letter-spacing: 0.2px;">
                        ...
                        <th width="35%" class="py-2">Kode & Nama Akun Sumber</th>
                ...
                <tfoot>
                    <tr style="background-color: color-mix(in srgb, var(--primary-accent) 4%, transparent);">
                        <td colspan="3" class="py-2 ps-4 text-end fw-bold">Total Penerimaan (Rp)</td>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top" style="border-color: rgba(226, 232, 240, 0.2) !important;">
        ...
        @if($transaction->status === 'draft')
            <x-button type="button" variant="primary" size="sm" class="btn-action-receipt" data-uuid="{{ $transaction->uuid }}" data-action="submit" icon="bi-send">
                Ajukan (Submit)
            </x-button>
        @elseif($transaction->status === 'submitted')
            <x-button type="button" variant="primary" size="sm" class="btn-action-receipt" data-uuid="{{ $transaction->uuid }}" data-action="approve" icon="bi-check-lg">
                Setujui (Approve)
            </x-button>
        @elseif($transaction->status === 'approved')
            <x-button type="button" variant="primary" size="sm" class="btn-action-receipt" data-uuid="{{ $transaction->uuid }}" data-action="post" icon="bi-check-circle-fill">
                Posting ke GL
            </x-button>
        @endif
    </div>
